...
object SkijaRenderer {
    ...
    private const val OVERLAY_LABEL = "Odin Skija GUI Overlay"
    ...
    init {
        try {
            defaultFont = Font(
                "Default",
                mc.resourceManager
                    .getResource(net.minecraft.resources.Identifier.parse("odin:font.ttf"))
                    .orElseThrow { FileNotFoundException("odin:font.ttf") }
                    .open()
            )
        } catch (e: IOException) {
    ...
    fun beginOverlayFrame(width: Float, height: Float): Boolean {
        ...
        val colorTexture = framebuffer.getColorTexture()
            ?: error("Odin Skija renderer cannot draw without an overlay color texture")
    ...
    private fun ensureOverlayTarget(mainTarget: RenderTarget): TextureTarget {
        val mainTexture = mainTarget.getColorTexture()
            ?: error("Odin Skija renderer cannot allocate an overlay without a main color texture")
    ...
    private fun ensureBackend(texture: GpuTexture): SkijaBackend {
        ...
        val next = when (texture) {
            ...
            else -> error("Odin Skija renderer does not support Minecraft texture backend ${texture.javaClass.name}")
        }
        backend = next
        return next
    }
    ...
    private fun radii(topLeft: Float, topRight: Float, bottomRight: Float, bottomLeft: Float): FloatArray {
        return floatArrayOf(
            topLeft, topLeft,
            topRight, topRight,
    ...
    fun rect(
        ...
        recordBounds(x, y, w, h)
        withPaint(color) { paint ->
            canvas().drawRRect(RRect.makeComplexXYWH(x, y, w, h + 0.5f, radii(topLeftRadius, topRightRadius, bottomRightRadius, bottomLeftRadius)), paint)
        }
    }
    ...
    fun hollowRect(
        ...
        withPaint(color, PaintMode.STROKE, thickness) { paint ->
            paint.setStrokeJoin(PaintStrokeJoin.ROUND)
            canvas().drawRRect(RRect.makeComplexXYWH(x, y, w, h, radii(topLeftRadius, topRightRadius, bottomRightRadius, bottomLeftRadius)), paint)
        }
    }
    ...
    fun text(text: String, x: Float, y: Float, size: Float, color: Int, font: Font = defaultFont) {
        try {
            ...
            OdinMod.logger.error(e.toString())
        }
    }
    ...
    private class VulkanSkijaBackend : SkijaBackend {
        ...
        override fun prepare(framebuffer: RenderTarget, texture: GpuTexture): Surface {
            check(texture is VulkanGpuTexture) {
                "Odin Skija Vulkan backend received ${texture.javaClass.name}"
            }
        ...
        private fun ensureContext(): DirectContext {
            ...
            check(backend is VulkanDevice) {
                "Odin Skija Vulkan backend requires Minecraft's Vulkan renderer"
            }
    ...
    private class OpenGlSkijaBackend : SkijaBackend {
        ...
        override fun prepare(framebuffer: RenderTarget, texture: GpuTexture): Surface {
            check(texture is GlTexture) {
                "Odin Skija OpenGL backend received ${texture.javaClass.name}"
            }
        ...
        private fun createFramebuffer(texture: GlTexture): Int {
            ...
            if (status != GL30C.GL_FRAMEBUFFER_COMPLETE) {
                ...
                error("Odin Skija OpenGL backend could not wrap Minecraft framebuffer: GL status 0x${status.toString(16)}")
            }
            return fbo
        }
    ...
    private fun getSkijaFont(font: Font, size: Float): SkijaFont {
        ...
        skFont.setEdging(FontEdging.ANTI_ALIAS)
